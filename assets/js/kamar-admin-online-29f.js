...
['admin-affiliate-overview.html','Data Affiliate'],
 ...
 function removeLegacyNotice(){
  ...
  function patchMemberRowCollapse(){if(!/^(admin-members|admin-internal)\.html$/i.test(PAGE))return;var wrap=qs('#adminMembersList');if(!wrap)return;function markRows(){wrap.querySelectorAll('.admin-row').forEach(function(row){if(row.classList.contains('header'))return;if(row.getAttribute('data-toggle-row'))return;row.setAttribute('data-toggle-row','1');row.classList.add('row-collapsed');var cells=row.children;if(cells[3])cells[3].classList.add('admin-row-hide');if(cells[4])cells[4].classList.add('admin-row-hide')})}markRows();if(!wrap.__collapseDelegated29F){wrap.__collapseDelegated29F=true;wrap.addEventListener('click',function(e){if(e.target.closest('a,button'))return;var row=e.target.closest('.admin-row[data-toggle-row]');if(!row)return;row.classList.toggle('row-collapsed')})}var obs=new MutationObserver(function(){markRows()});obs.observe(wrap,{childList:true,subtree:true})}
  async function run(){
    if(!/^admin/i.test(PAGE)) return;
    injectSharedStyles();
    rebuildSidebar();
    await patchKamarAdminLocal(); await patchMemberDeleteButtons(); await patchMemberRowCollapse();
