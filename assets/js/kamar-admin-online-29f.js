...
['admin-affiliate-overview.html','Data Affiliate'],
 ...
 function removeLegacyNotice(){
  ...
  function patchMemberRowCollapse(){if(!/^(admin-members|admin-internal)\.html$/i.test(PAGE))return;var wrap=qs('#adminMembersList');if(!wrap)return;function ensureCollapse(){wrap.querySelectorAll('.admin-row').forEach(function(row){if(row.classList.contains('header'))return;if(row.getAttribute('data-toggle-row'))return;row.setAttribute('data-toggle-row','1');row.classList.add('row-collapsed');var cells=row.children;if(cells[3])cells[3].classList.add('admin-row-hide');if(cells[4])cells[4].classList.add('admin-row-hide');row.addEventListener('click',function(e){if(e.target.closest('a,button'))return;row.classList.toggle('row-collapsed')})})}ensureCollapse();var obs=new MutationObserver(function(){ensureCollapse()});obs.observe(wrap,{childList:true,subtree:true})}
  async function run(){
