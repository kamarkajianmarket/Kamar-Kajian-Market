(function(){
  ...
  var SIDEBAR_SECTIONS = [    { label:'Utama', items:[ ['admin.html','Dashboard Admin'] ] },    { label:'Token Admin', items:[ ['admin-token.html','Token Admin'] ] },    { label:'Member & Akses', items:[      ['admin-members.html','Data Member'],      ['admin-internal.html','Data Internal'],      ['admin-activation.html','Aktivasi Akun & Fasilitas'],      ['admin-license-requests.html','Request License']    ] },    { label:'Affiliate', items:[      ['admin-affiliate-overview.html','Data Affiliate'],      ['admin-affiliate-list.html','Daftar Affiliator'],      ['admin-affiliate-payment.html','Pencairan Komisi'],      ['admin-affiliate-commission-rate.html','Pengaturan Komisi'],      ['admin-affiliate-reward.html','Reward / Komisi (Lama)']      ] },    { label:'Konten Website', items:[      ['admin-banner.html','Banner Pengumuman'],      ['admin-video.html','Konten Video'],      ['admin-materials.html','Konten & Materi'],      ['admin-tools.html','File Tools']      ] },    { label:'Kontrol Sistem', items:[      ['admin-maintenance.html','Maintenance Fasilitas'],      ['admin-page-control.html','Kontrol Halaman Utama'],      ['admin-dashboard-control.html','Kontrol Dashboard Member'],      ['admin-payment.html','Payment Gateway'],      ['admin-payment-products.html','Katalog Harga & Produk'],      ['admin-links.html','Pengaturan Link'],      ['admin-settings.html','Pengaturan Umum']      ] },    { label:'Akun', items:[ ['index.html','Logout'] ] }  ];
...
async function actionProfileChange(todo, approve){
  ...
  async function actionDownloadRequest(todo, approve){ var payload = todo.action_payload || {}; var pid = todo.profile_id || await resolveProfileId(todo); if(pid && payload.file_id){ var c2 = await ready(); if(c2){ var res2 = await c2.from('kamar_download_requests').select('id').eq('member_profile_id', pid).eq('file_id', payload.file_id).eq('request_status', 'pending').order('created_at',{ascending:false}).limit(1); if(res2.error) throw res2.error; if(res2.data && res2.data.length){ var reqId = res2.data[0].id; if(approve){ await update('kamar_download_requests', reqId, { request_status:'approved', reviewed_at:new Date().toISOString(), approved_expires_at:new Date(Date.now()+48*3600*1000).toISOString() }); } else { await update('kamar_download_requests', reqId, { request_status:'rejected', reviewed_at:new Date().toISOString() }); } } } } await markTodoDone(todo.id, approve?'done':'rejected'); notifyMemberForTodo(todo, approve?'Request Download Disetujui':'Request Download Ditolak', approve?('Request download "'+(payload.file_title||'file')+'" sudah disetujui admin. Klik tombol Download di halaman '+(payload.facility_label||'Fasilitas')+' Anda dalam 48 jam sebelum kedaluwarsa.'):('Request download "'+(payload.file_title||'file')+'" ditolak admin. Hubungi admin untuk info lebih lanjut.'), { link: payload.facility_key==='kamar_indikator' ? 'member-indicator.html' : 'member-robot.html', category:'download' }); toast(approve?'Request download disetujui.':'Request download ditolak.'); } async function actionIbKamarApplication(todo, action){
    ...
    if(appId){
      var c = await ready();
      if(c){
        var overrideDur = action === 'approve' ? getReviewDurationOverride(todo.id) : null;
        var res = await c.rpc('admin_review_ib_kamar_application', { p_application_id: appId, p_action: action, p_duration_days: overrideDur, p_admin_note: note });
        if(res.error) throw res.error;
      }
    }
...
actionsHtml = reviewBtn+'<button class="btn mini" type="button" data-todo-action="approve_affiliate_activation" data-todo-id="'+esc(todo.id)+'">Setujui</button> <button class="btn mini secondary" type="button" data-todo-action="reject_affiliate_activation" data-todo-id="'+esc(todo.id)+'">Tolak</button>';
  ...
  function getReviewNote(todoId){
    var input = document.getElementById('kamarReviewNoteInput29F');
    if(!input || input.dataset.todoId !== String(todoId)) return null;
  ...
  function openReviewModal(todo){
    ...
    if(NOTE_EDIT_TYPES[todo.todo_type]){
      html += '<div class="kamar-review-duration">'+
        ...
        '<textarea id="kamarReviewNoteInput29F" data-todo-id="'+esc(todo.id)+'" rows="3" placeholder="Contoh: nomor akun trading yang diisi belum sesuai, mohon periksa kembali." style="width:100%;box-sizing:border-box;background:#1c1a16;border:1px solid rgba(238,206,122,.3);border-radius:10px;color:#f5f0e6;font-size:14px;font-weight:600;padding:9px 12px;resize:vertical"></textarea>'+
        '<small>Wajib diisi kalau menekan "Kembalikan untuk Revisi" - member akan melihat catatan ini persis seperti yang diketik. Opsional untuk Setujui.</small>'+
      '</div>';
    }
...
(function(){
  ...
  function injectNotifStyles(){
    ...
    st.textContent = '#kamarNotifBtn29F.kamar-notif-btn{position:relative;display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;padding:0!important;box-sizing:border-box!important;border-radius:12px;border:1px solid rgba(120,90,20,.25);background:linear-gradient(135deg,#f4df90,#c69a39);color:#1c1a16;cursor:pointer;font-size:18px;box-shadow:0 3px 10px rgba(184,138,61,.45)}'
