(function(){
  ...
  function statusBadge(){var ok=(state.viewRows.length||state.profileRows.length); if(ok)return '<span class="status-pill on">Supabase Online</span>'; if(state.error)return '<span class="status-pill warn">Supabase belum terbaca</span>'; return '<span class="status-pill off">Data lokal</span>'}
  function dbLine(){return statusBadge()+' <span class="kamar-chip">member_profiles: '+state.profileRows.length+'</span> <span class="kamar-chip">admin_member_overview: '+state.viewRows.length+'</span>'+(state.error?'<div class="admin-empty-state" style="margin-top:10px">'+esc(state.error)+'</div>':'')}
  ...
  function findCurrentMember(){var s=session('member')||{}; var em=norm(emailOf(s)||s.email); var mid=norm(idOf(s)||s.memberId||s.id); var row=state.members.find(function(m){return (em&&norm(emailOf(m))===em)||(mid&&norm(idOf(m))===mid)||(mid&&norm(pick(m,['user_id','profile_id','id']))===mid)}) || state.members[0] || s; if(row&&emailOf(row)){var newS=Object.assign({},s,row,{fullName:nameOf(row),email:emailOf(row),memberId:idOf(row)}); setJSON('kamarMemberSession',newS); setJSON('KAMAR_MEMBER_SESSION',newS)} return row; }
  ...
  function memberDetailsHTML(m){var fac=facilitiesOf(m); return '<div class="admin-detail-panel">'+
    ...
    '</div>'; }
  function fillMemberDashboard(){if(!/dashboard\.html$/i.test(location.pathname))return; var m=findCurrentMember(); if(!m)return; setText('memberTitle','Selamat datang, '+nameOf(m)); var mt=el('memberText'); if(mt)mt.textContent='Dashboard akun member Kamar Kajian Market. Data di bawah dibaca dari database online jika koneksi Supabase tersedia.'; var line=el('memberSessionLine'); if(line)line.innerHTML='Login sebagai: '+esc(emailOf(m)||'-')+'<br>'+dbLine(); var hero=qs('.hero')||qs('.split-main'); if(hero&&!el('memberOnlineDetailCard')){var d=document.createElement('div');d.className='card';d.id='memberOnlineDetailCard';d.innerHTML='<span class="eyebrow">Detail Akun Online</span>'+memberDetailsHTML(m);hero.insertAdjacentElement('afterend',d)} }
  function fillMemberProfile(){if(!/member-profile\.html$/i.test(location.pathname))return; var m=findCurrentMember(); if(!m)return; setText('profileFullName',nameOf(m)); setText('profileMemberId',idOf(m)); setText('profileEmail',emailOf(m)); setText('profileWhatsapp',waOf(m)); setText('profileTelegram',tgOf(m)); setText('profileStatus',isActive(m)?'Aktif':isPending(m)?'Pending Aktivasi':'Belum Aktif'); setText('profileAccessDate',expiryOf(m)); setText('profileFacilities',facilitiesOf(m).join(', ')||'Belum ada fasilitas aktif'); setText('profileReferralCode',referralOf(m)||'-'); var cur=el('profileReferralCurrentCode'); if(cur)cur.textContent='REFERRAL: '+(referralOf(m)||'-'); var input=el('memberReferralInput'); if(input&&referralOf(m))input.value=referralOf(m); var title=qs('.kamar-title-card'); if(title&&!el('memberOnlineStatus25e')){var p=document.createElement('div');p.id='memberOnlineStatus25e';p.className='button-row';p.innerHTML=dbLine();title.appendChild(p)} }
  ...
  async function run(){attachLogoutOnly(); var protectedPage=/admin|dashboard|member-/i.test(location.pathname); if(protectedPage){ try{await loadAll()}catch(e){state.error=e.message; state.source='error'} fillMemberDashboard(); fillMemberProfile(); fillAdminDashboard(); fillAdminMembers(); injectAffiliateMenu(); } }
