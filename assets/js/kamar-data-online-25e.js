...
var hero=qs('.hero')||qs('.split-main'); if(hero&&!el('memberOnlineDetailCard')){var d=document.createElement('div');d.className='card';d.id='memberOnlineDetailCard';d.innerHTML='<span class="eyebrow">Detail Akun Online</span>'+memberDetailsHTML(m);hero.insertAdjacentElement('afterend',d)}
}
  function fillMemberProfile(){if(!/member-profile\.html$/i.test(location.pathname))return; var m=findCurrentMember(); if(!m)return; setText('profileFullName',nameOf(m)); setText('profileMemberId',idOf(m)); setText('profileEmail',emailOf(m)); setText('profileWhatsapp',waOf(m)); setText('profileTelegram',tgOf(m)); setText('profileStatus',isActive(m)?'Aktif':isPending(m)?'Pending Aktivasi':'Belum Aktif'); setText('profileAccessDate',expiryOf(m)); setText('profileFacilities',facilitiesOf(m).join(', ')||'Belum ada fasilitas aktif'); setText('profileReferralCode',referralOf(m)||'-'); setText('profileBroker',brokerOf(m)||'-'); setText('profileModal',capitalOf(m)||'-'); var cur=el('profileReferralCurrentCode'); if(cur)cur.textContent='REFERRAL: '+(referralOf(m)||'-'); var input=el('memberReferralInput'); if(input&&referralOf(m))input.value=referralOf(m); try{document.dispatchEvent(new CustomEvent('kamarProfileLoaded',{detail:{member:m}}))}catch(e){} var title=qs('.kamar-title-card'); if(title&&!el('memberOnlineStatus25e')){var p=document.createElement('div');p.id='memberOnlineStatus25e';p.className='button-row';p.innerHTML=dbLine();title.appendChild(p)} }
  ...
  async function run(){attachLogoutOnly(); var protectedPage=/admin|dashboard|member-/i.test(location.pathname); if(protectedPage){ try{await loadAll()}catch(e){state.error=e.message; state.source='error'} fillMemberDashboard(); fillMemberProfile(); fillAdminDashboard(); fillAdminMembers(); } }
