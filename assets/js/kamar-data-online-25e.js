(function(){
 ...
 function qs(s,r){return (r||document).querySelector(s)}
  ...
  function findCurrentMember(){var s=session('member')||{}; var em=norm(emailOf(s)||s.email); var mid=norm(idOf(s)||s.memberId||s.id); var matchFn=function(m){return (em&&norm(emailOf(m))===em)||(mid&&norm(idOf(m))===mid)||(mid&&norm(pick(m,['user_id','profile_id','id']))===mid)}; var row=state.members.find(matchFn) || state.internal.find(matchFn) || s; if(row&&emailOf(row)){var newS=Object.assign({},s,row,{fullName:nameOf(row),email:emailOf(row),memberId:idOf(row)}); setJSON('kamarMemberSession',newS); setJSON('KAMAR_MEMBER_SESSION',newS)} return row; }
...
var hero=qs('.hero')||qs('.split-main'); if(hero&&!el('memberOnlineDetailCard')){var d=document.createElement('div');d.className='card';d.id='memberOnlineDetailCard';d.innerHTML='<span class="eyebrow">Detail Akun Online</span>'+memberDetailsHTML(m);(hero.classList.contains('split-main')?hero.appendChild(d):hero.insertAdjacentElement('afterend',d))}
  ...
  function revealProfileGates(){ if(window.KamarUI && window.KamarUI.revealAll) window.KamarUI.revealAll('.profile-panel.kamar-skel-gate'); }
  function fillMemberProfile(){if(!/member-profile\.html$/i.test(location.pathname))return; var m=findCurrentMember(); if(!m){ revealProfileGates(); return; } setText('profileFullName',nameOf(m)); setText('profileMemberId',idOf(m)); setText('profileEmail',emailOf(m)); setText('profileWhatsapp',waOf(m)); setText('profileTelegram',tgOf(m)); setText('profileStatus',isActive(m)?'Aktif':isPending(m)?'Pending Aktivasi':'Belum Aktif'); setText('profileAccessDate',fmtAccessDate(expiryOf(m))); setText('profileFacilities',facilitiesOf(m).join(', ')||'Belum ada fasilitas aktif'); setText('profileReferralCode',referralOf(m)||'-'); setText('profileBroker',brokerOf(m)||'-'); setText('profileModal',capitalOf(m)||'-'); var cur=el('profileReferralCurrentCode'); if(cur)cur.textContent='REFERRAL: '+(referralOf(m)||'-'); var input=el('memberReferralInput'); if(input&&referralOf(m))input.value=referralOf(m); try{document.dispatchEvent(new CustomEvent('kamarProfileLoaded',{detail:{member:m}}))}catch(e){} var title=qs('.kamar-title-card'); if(title&&!el('memberOnlineStatus25e')){var p=document.createElement('div');p.id='memberOnlineStatus25e';p.className='button-row';p.innerHTML=dbLine();title.appendChild(p)} revealProfileGates(); }
