(function () {
  ...
  async function loadMemberStudyLiveCard() {
    ...
    try {
      ...
      box.innerHTML = '<div class="panel-head"><div><span class="mini-label">Card Utama Kamar Study</span><h3>' + escapeHtml(x.pair || "-") + ' · ' + escapeHtml(x.timeframe || "-") + '</h3></div><span class="status-pill ' + (studyStatus(x) === "Fresh" ? "fresh" : "running") + '">' + escapeHtml(studyStatus(x)) + '</span></div>'+
        ...
        '<div class="signal-two-rails"><div class="rail-card left-rail"><div><span>Harga Saat Ini</span><strong class="live-price">' + formatPrice(x.current_price) + '</strong></div><div><span>Area Kajian</span><strong>' + formatPrice(x.area_low) + ' – ' + formatPrice(x.area_high) + '</strong></div><div><span>Invalidasi</span><strong>' + formatPrice(x.invalidasi) + '</strong></div></div>'+
        '<div class="rail-card right-rail progress-rail"><div><span>Update</span><strong>' + escapeHtml(progressLabel(x)) + '</strong></div><div><span>Running Actual</span><strong class="' + pc + '">' + formatPoint(x.running_point) + '</strong></div><div><span>Running Terjauh</span><strong class="' + mc + '">' + formatPoint(x.max_running_point) + '</strong></div></div></div>'+
  ...
  async function loadMemberStudyMainCard(){
    ...
    try{
      ...
      box.innerHTML='<span class="mini-label">Kamar Study Terdekat</span><div class="admin-detail-row"><div><h2>'+mdEsc(x.pair||'-')+' · '+mdEsc(x.timeframe||'-')+'</h2><p>'+mdEsc(x.skenario||'-')+' Study · '+mdEsc(x.jenis_zona||'-')+' · Status Zona: '+mdEsc(mdStatus(x))+'</p><div class="study-live-meta"><span>Harga Saat Ini: <strong>'+mdPrice(x.current_price)+'</strong></span><span>Running Actual: <strong class="'+mdPointClass(x.running_point)+'">'+mdPoint(x.running_point)+'</strong></span><span>Running Terjauh: <strong class="'+mdPointClass(x.max_running_point)+'">'+mdPoint(x.max_running_point)+'</strong></span></div><p class="study-live-note">ID Zona: '+mdEsc(x.id_zona||'-')+'</p></div><a class="btn secondary" href="member-study.html">Lihat Detail Kamar Study</a></div>';
